...
## デコメントしよう
bflute_maihamadb/schema/decomment/piece/
タブを二つ作り、同時に修正してコンフリクトさせた(schemaが赤くなった)
...
## DB設計のレビューを自動化
①には「同じデータが2つ以上存在することを防ぐため、全テーブルにPKを必須とする」                                                                                                                                                         
②には「環境依存をなくすため、テーブル/カラム名は全て大文字で統一する」                                                                                                                                                                 
③には「タイムゾーン系のバグを減らすため、日付は DATE、日時は DATETIMEで統一する」

## WebアプリでのDBFluteを知ろう
lastafluteというwebフレームワークをつかっていて、これが初登場なので何もわからない
